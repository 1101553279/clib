...
#define log_print(fmt, args...)	do{ printf(fmt, ##args); }while(0)
#define logc_print(color, fmt, args...)  do{ printf(color fmt COLOR_FONT_NONE, ##args);} while(0)

#define log_red_print(fmt, args...)	do{ printf(COLOR_FONT_RED fmt COLOR_FONT_NONE, ##args);}while(0)
#define log_grn_print(fmt, args...)	do{ printf(COLOR_FONT_GREN fmt COLOR_FONT_NONE, ##args);}while(0)
#define log_yel_print(fmt, args...)	do{ printf(COLOR_FONT_YELL fmt COLOR_FONT_NONE, ##args);}while(0)
#define log_whi_print(fmt, args...)	do{ printf(COLOR_FONT_WHIT fmt COLOR_FONT_NONE, ##args);}while(0)

#define log_blk_print(fmt, args...)	do{ printf(PRINT_ATTR_BLNK fmt PRINT_ATTR_UBLK, ##args); }while(0)
...
#if CONFIG_LOG && defined(CONFIG_LOG_ERROR_RETURN)
/*
...
struct log_driver {
    ...
    int (*probe)(struct log_device *dev);
    void (*remove)(struct log_device *dev);
    
    struct list_head device_head;   /* for log_device list */
    struct list_head sibling_node;  /* for log_driver list */
...
struct log_device {
    ...
    struct list_head driver_node;   /* for device_head of the log_driver */
...
static inline void *log_device_get_drvdata(struct log_device *dev)
{
    return dev->drv_data;
}
...
#define CONFIG_LOGF_FILE    1
#define CONFIG_LOGF_LINE    1
#define CONFIG_LOGF_FUNC    1
...
{
	return (0x1<<LOGF_MSG) |
	       (CONFIG_LOGF_FILE ?(0x1<<LOGF_FILE) : 0) |
	       (CONFIG_LOGF_LINE ?(0x1<<LOGF_LINE) : 0) |
	       (CONFIG_LOGF_FUNC ?(0x1<<LOGF_FUNC) : 0);
}
